...
class RPCGameClient():
	...
	self.__broadcast_receiver = RPCGameClient.BroadcastReceiver(brcst_callback, brcst_server_address, brcst_server_port)
    ...
    def new_player(self, nickname):
        try:
            if nickname is not None:
                self.__client_id = JSONDecoder().decode(self.__proxy.new_player(nickname, self.__br_ip, self.__br_port))["client_id"]
                return {}
